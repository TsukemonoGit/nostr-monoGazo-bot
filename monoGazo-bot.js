...
//---------------------------------------------func------------------------------------------------
function monoGazo(packet, urlIndex) {
  ...
  postEvent(packet.event.kind, `#もの画像\n${urlList[urlIndex].url}\n作: nostr:${urlList[urlIndex].author} ${urlList[urlIndex].memo ? " (" + urlList[urlIndex].memo + ")" : ""}`, tags);
...
function profileChange(packet) {
  ...
  postEvent(1, `あいこんかえた\n${urlList[urlIndex].url}\n作: nostr:${urlList[urlIndex].author} ${urlList[urlIndex].memo ? " (" + urlList[urlIndex].memo + ")" : ""}`, [["r", urlList[urlIndex].url]]);
...
function wareki(packet) {
  ...
  postEvent(packet.event.kind, `${wareki} らしい`, tags,created_at);
...
function monoLen(packet) {
  ...
  postEvent(packet.event.kind, `もの画像は今全部で${urlList.length-1}枚あるよ`, tags);
}
